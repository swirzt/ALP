...
binary2Bound v e1 e2 f = let ee1 = expStr2Bound' v e1
                             ee2 = expStr2Bound' v e2
                         in f ee1 ee2

unary2Bound :: [String] -> Exp String -> (Exp Vars -> g Vars) -> g Vars
unary2Bound v e f = let ee = expStr2Bound' v e
                    in f ee
...
bool2Bound v e1 e2 f = let ee1 = boolStr2Bound' v e1
...
commStr2Bound' v (Rep e xs) = let ys = map (commStr2Bound' v) xs
                                  ee = expStr2Bound' v e
                              in Rep ee ys
...
commStr2Bound' v (ForDelta str e1 e2 e3 xs) = let ys = map (commStr2Bound' v) xs
                                                  ee1 = expStr2Bound' v e1
                                                  ee2 = expStr2Bound' v e2
                                                  ee3 = expStr2Bound' v e3
                                              in ForDelta str ee1 ee2 ee3 ys
commStr2Bound' v (For str e1 e2 xs) = let ys = map (commStr2Bound' v) xs
                                          f = \x y -> For str x y ys
                                      in binary2Bound v e1 e2 f
commStr2Bound' v (If b xs) = let ys = map (commStr2Bound' v) xs
                                 f = \x _ -> If x ys
                             in bool2Bound v b b f
commStr2Bound' _ Fill = Fill
commStr2Bound' v (Filled e xs) = let ys = map (commStr2Bound' v) xs
                                     f = \x -> Filled x ys
                                 in unary2Bound v e f
commStr2Bound' v (Wait e) = unary2Bound v e Wait
commStr2Bound' v (While xs b) = let ys = map (commStr2Bound' v) xs
                                in bool2Bound v b b (While ys .* const)
commStr2Bound' v (CommVar str xs) = let ys = listStr2Bound' v xs
                                    in CommVar str ys
